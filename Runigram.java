...
public class Runigram {
	...
	public static Color[][] flippedVertically(Color[][] image) {
		...
		for (int i = 0; i < image[0].length; i++) {
			for (int j = 0; j < image.length; j++) {
				flipped[i][j] = image[image[0].length - j - 1][i];
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		int ogWid = image.length;
		int ogHei = image[0].length;
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				scaled[i][j] = image[j * ogHei / height][i * ogWid / width];
	...
	public static Color[][] blend(Color[][] image1, Color[][] image2, double alpha) {
		...
		for (int i = 0; i < image1.length; i++) {
			for (int j = 0; j < image1[0].length; j++) {
				Color res = blend(image1[i][j], image2[i][j], alpha);
				blended[i][j] = res;
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		...
		for (int i = 0; i < n; i++) {
			Color res[][] = blend(source, target, (double) ((n - i) / n));
